...
async function forceRegenerate() {
  ...
  console.log("API Key found in env:", !!process.env.REQUESTY_API_KEY);
  if (process.env.REQUESTY_API_KEY) {
      console.log("API Key prefix:", process.env.REQUESTY_API_KEY.substring(0, 12));
  }
  ...
  try {
    ...
    if (publishResult.success) {
      ...
      const platform = publishResult.platform || "notion";
