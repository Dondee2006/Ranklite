...
async function publishNow() {
  ...
  const integrationId = 'e77e62a2-bd9d-4920-a6d7-0a02d922190d';
  ...
  for (const article of articles) {
    ...
    try {
      ...
      if (data.success) {
        // Update status to published manually since we're bypassing the cron's auto-update
        // Actually the API doesn't update the status, the cron does.
        // So I'll update it here or in the API.
      }
    } catch (error) {
