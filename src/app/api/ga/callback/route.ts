...
export async function GET(request: NextRequest) {
    try {
        ...
        const redirectUri = process.env.GOOGLE_REDIRECT_URI || `${process.env.NEXT_PUBLIC_APP_URL}/api/ga/callback`;
        ...
        if (dbError) {
            console.error("Failed to store GA tokens:", dbError);
            return NextResponse.redirect(
        ...
        return NextResponse.redirect(
            new URL("/dashboard/integrations?success=true&integration=ga", request.url)
        );
    } catch (error) {
